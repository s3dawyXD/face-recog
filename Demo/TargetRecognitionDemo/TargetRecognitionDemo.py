...
from TargetRecognitionUI import Ui_MainWindow
...
class CallBackAlarmInfo:
    ...
    def get_detect_info(self, alarm_info, is_face):
        if is_face:
            ...
            self.face_color_str = '未知(UNKNOWN)'
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def _init_ui(self):
        ...
        self.Play_pushButton.setText('预览(Play)')
        self.Play_pushButton.setEnabled(False)

        self.IP_lineEdit.setText('192.168.100.43')
        self.Port_lineEdit.setText('80')
        self.Name_lineEdit.setText('admin')
        self.Pwd_lineEdit.setText('LA7312@TA')
    ...
    def login_btn_onclick(self):
        ...
        if not self.loginID:
            ...
            if self.loginID != 0:
                self.setWindowTitle('目标识别(TargetRecognition)-在线(OnLine)')
            ...
            if result:
                self.setWindowTitle("目标识别(TargetRecognition)-离线(OffLine)")
                self.Login_pushButton.setText("登录(Login)")
                self.loginID = 0
                self.Play_pushButton.setEnabled(False)
    ...
    def show_recognition_info(self, show_info, num, is_global, is_face, is_candidate):
        ...
        if not self.realloadID:
            return
        if is_global:
            self.GlobalImg_groupBox.setTitle('全景图(Global Picture)--目标识别(TargetRecognition)')
        ...
        if is_face:
            self.FaceImg_groupBox.setTitle('人脸图(Face Picture)--目标识别(TargetRecognition)')
        ...
        if is_candidate:
            self.CandidateImg_groupBox.setTitle('候选图(Candidate Picture)--目标识别(TargetRecognition)')
    ...
    def show_detect_info(self, show_info, num, is_global, is_face):
        if not self.realloadID:
            return
        if is_global:
            self.GlobalImg_groupBox.setTitle('全景图(Global Picture)--目标检测(FaceDetect)')
            image = QPixmap('./Global_Detect/Global_Img'+str(num)+ '.jpg').scaled(self.GlobalImg_wnd.width(), self.GlobalImg_wnd.height())
            self.GlobalImg_wnd.setPixmap(image)
        if is_face:
            self.FaceImg_groupBox.setTitle('人脸图(Face Picture)--目标检测(FaceDetect)')
